...
export default class BidirectionalFolgezettelPlugin extends Plugin {
    ...
    async onload() {
        ...
        this.registerEvent(
            this.app.vault.on('create', (file) => {
                if (this.settings.autoProcess && file instanceof TFile) {
    ...
    async createNextChild() {
        ...
        const childFileName = `${childAddress}.md`;

        // Create the child file in the same folder as parent
        const parentFolder = activeView.file.parent;
        const childPath = parentFolder 
            ? `${parentFolder.path}/${childFileName}`
            : childFileName;

        try {
            const childFile = await this.app.vault.create(childPath, '');
            await this.app.workspace.getLeaf().openFile(childFile);
            new Notice(`Created child note: ${childAddress}`);
        } catch (error) {
    ...
    async insertCrossLink(file: TFile, linkedFile: TFile) {
        ...
        try {
            ...
            if (inserted && this.settings.showNotifications) {
                new Notice(`Reciprocal link added: ${linkedFile.basename} → ${file.basename}`);
            }
        } finally {
...
class FolgezettelSettingTab extends PluginSettingTab {
    ...
    display(): void {
        ...
        new Setting(containerEl)
            ...
            .setDesc('Automatically create reciprocal links when you link to another note')
            .addToggle(toggle => toggle
