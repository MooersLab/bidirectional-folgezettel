...
describe("BidirectionalFolgezettelPlugin", () => {
  ...
  describe("suggestNextChild", () => {
    it('should suggest "a" for first child', () => {
      const next = plugin.suggestNextChild("1.2");
      expect(next).toBe("1.2a");
    });

    it("should suggest next letter when children exist", () => {
    ...
    it("should find highest letter among children", () => {
      mockVault.addTestFile("1.2 Parent.md");
  ...
  describe("insertLinkUnderHeading", () => {
    it("should insert link under existing heading", async () => {
      const file = mockVault.addTestFile(
        "test.md",
        "# Title\n\n## Related Notes\n",
      );
      const linkedFile = new TFile("linked.md");

      await plugin.insertLinkUnderHeading(
        file as any,
        linkedFile as any,
        "Related Notes",
        "Test",
    ...
    it("should not duplicate existing link", async () => {
      ...
      const inserted = await plugin.insertLinkUnderHeading(
        file as any,
        linkedFile as any,
        "Related Notes",
        "Test",
