...
var BidirectionalFolgezettelPlugin = class extends import_obsidian.Plugin {
  ...
  async onload() {
    ...
    this.registerEvent(
      this.app.vault.on("create", (file) => {
        if (this.settings.autoProcess && file instanceof import_obsidian.TFile) {
  ...
  async createNextChild() {
    ...
    const childPath = parentFolder ? `${parentFolder.path}/${childFileName}` : childFileName;
    try {
      const childFile = await this.app.vault.create(childPath, "");
      await this.app.workspace.getLeaf().openFile(childFile);
      new import_obsidian.Notice(`Created child note: ${childAddress}`);
    } catch (error) {
      new import_obsidian.Notice(`Failed to create child note: ${error}`);
    }
  }
  // ========================================================================
